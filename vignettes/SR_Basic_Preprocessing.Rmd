...
It is common to export a period of time prior to the onset of the stimulus as a baseline. In this case, an offset (equal to the duration of the baseline period) must be applied to the time series, specified in the `Adj` parameter. 
...
Note that if you have used the `align_msg` function (illustrated in the `SR_Align_Message` vignette), you may need to specify a column name in `Adj`. 
That column can be used to apply offsets specific to each recording event. 
Consult the vignette for further details.
